...
def list_interfaces():
   ...
   for i in range(0, outbytes, struct_size):
      interfaces.append((namestr[i:i+16].split(b'\0')[0]))
   return interfaces

if __name__ == '__main__':
   interfaces = list_interfaces()
   for i in range(len(interfaces)):
      interfaces[i] = interfaces[i].decode()
   print("This machine has {} network interfaces: {}".format(str(len(interfaces)), interfaces[::], sep=','))